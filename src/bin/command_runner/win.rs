...
use yj_sandbox::{
    ConsoleMode, ErrorPayload, ErrorStage, ExitPayload, FramedMessage, IPC_PROTOCOL_VERSION,
    JobObject, LocalSid, Message, OutputPayload, OutputStream, PipeSpawnHandles, ResizePayload,
    SpawnReady, SpawnRequest, StderrMode, StdinMode, WindowsSandboxTokenMode, allow_null_device,
    create_readonly_token_with_caps_and_user_from,
    create_workspace_write_token_with_caps_and_user_from, decode_bytes, encode_bytes,
    get_current_token_for_restriction, hide_current_user_profile_dir, log_note, read_frame,
    read_handle_loop, spawn_process_with_pipes, to_wide, write_frame,
};
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<yj_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    ...
    let token_mode = req
        .permissions
        .token_mode_for_cwd(&req.cwd, &req.env)
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = yj_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
