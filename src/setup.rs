...
impl SandboxNetworkIdentity {
    pub(crate) fn from_permissions(
        ...
        if proxy_enforced || permissions.should_apply_network_block() {
            Self::Offline
        } else {
            Self::Online
        }
...
// Upstream's tests below construct `codex_protocol::PermissionProfile` values.
// The production setup code uses the resolved local adapter, so these fixtures
// stay beside upstream for syncing but are not built in this standalone crate.
#[cfg(all(test, any()))]
mod tests {
    ...
    #[test]
    fn setup_marker_ignores_port_drift_while_local_binding_stays_enabled() {
        let marker = super::SetupMarker {
            version: super::SETUP_VERSION,
            offline_username: "offline".to_string(),
            online_username: "online".to_string(),
