...
struct RawConPty {
    pseudoconsole: PsuedoCon,
    input_write: OwnedHandle,
    output_read: OwnedHandle,
}

impl RawConPty {
    fn new(cols: i16, rows: i16) -> Result<Self> {
        ...
        if result < 0 {
            ...
            anyhow::bail!("CreatePseudoConsole failed: HRESULT {result}");
        }

        Ok(Self {
            pseudoconsole: PsuedoCon {
                handle,
                _input_read: unsafe { OwnedHandle::from_raw_handle(input_read as _) },
                _output_write: unsafe { OwnedHandle::from_raw_handle(output_write as _) },
            },
            input_write: unsafe { OwnedHandle::from_raw_handle(input_write as _) },
            output_read: unsafe { OwnedHandle::from_raw_handle(output_read as _) },
        })
    }

    fn into_handles(self) -> (PsuedoCon, OwnedHandle, OwnedHandle) {
        (self.pseudoconsole, self.input_write, self.output_read)
    }
}
